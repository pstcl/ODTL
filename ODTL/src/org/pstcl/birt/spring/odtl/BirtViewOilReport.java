package org.pstcl.birt.spring.odtl;
...
public class BirtViewOilReport extends AbstractView {
	...
	protected void renderMergedOutputModel(
		...
		if( reportFormat.equalsIgnoreCase("html")){    
			...
			//pdfOptions.setOutputFileName("d://testfile");
	...
	public  void saveReportToDb(
			...
			ReportFile file,ServletContext context) throws Exception {
		//ServletContext sc = request.getSession().getServletContext();

		IReportRunnable runnable = null;
		runnable = birtEngine.openReportDesign( context.getRealPath("/Reports")+"/"+reportName );
		IRunAndRenderTask runAndRenderTask = birtEngine.createRunAndRenderTask(runnable);
		...
		pdfOptions.setOutputStream(arrayOutputStream);
		//pdfOptions.setOutputStream(response.getOutputStream());

		//pdfOptions.setOutputStream(file.getPdfFile());
		...
		file.setPdfFile(arrayOutputStream.toByteArray());
